...
| `data/simulated/recipe/laser_sensor.json` | Simulated shaped 1×3 `["G","A","C"]` с `minecraft:tinted_glass` (G) + `#simulated:laser_point_lens` (A, сводится к `#c:gems/amethyst` — единственный item в теге) + `create:andesite_casing` (C) → 1 `simulated:laser_sensor` | тот же pattern, ключ `A = #tfc_aeronautics:gem` → 1 `simulated:laser_sensor`. Мотивация: `#simulated:laser_point_lens` фактически сводится к аметисту — в TFC-сборке аметист труднодоступен (нет `amethyst_geode`); замена на `#tfc_aeronautics:gem` (9 TFC-гемов) открывает craft-путь через TFC-металлургию (anvil). По образцу `optical_sensor.json` (тот же свап). `show_notification: false` (конвенция). **Ветка 1** скилла `recipe-override` (recipe-id в namespace `simulated`, без `BANNED_RECIPES`). Шейдинг-тегов не требуется: `#tfc_aeronautics:gem` уже создан в нашем namespace. Recipe-id `simulated:laser_sensor` сохраняется, advancement `data/simulated/advancement/recipes/misc/laser_sensor.json` ссылается на тот же id — засчитывается без правок. `simulated:laser_pointer.json` и тег `simulated:laser_point_lens` намеренно не трогаем — пользователь просил переделать только `laser_sensor` |
