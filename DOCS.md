...
| `data/create/recipe/crafting/logistics/packager.json` | Create shaped 3×3 `[" C ","CAC","RCR"]`: A=`create:cardboard_block` (центр, 1×) + C=`#c:ingots/iron` (4× — крестом вокруг центра) + R=`#c:dusts/redstone` (2× — нижние углы) → 1 `create:packager` (recipe-id `create:crafting/logistics/packager`). В TFC-сборке мёртв: `c:ingots/iron` пуст для TFC-wrought_iron_sheet (TFC хранит металлы в `tfc:metal/<form>/<type>` и не регистрирует листы в common-тегах `c:plates/*`); `c:dusts/redstone` для `minecraft:redstone` избыточно толерантен (тянет любые модовые «dust» аналоги) | TFC-style shaped 3×2 `["SCS","R R"]`: S=`tfc:metal/sheet/wrought_iron` (2×) + C=`create:cardboard_block` (1×) + R=`minecraft:redstone` (2×) → 1 `create:packager`. Мотивация: фиксируем точные ингредиенты — кованое железо по двум верхним углам решётки, картонный блок в центре, ванильный редстоун по нижним углам. Pattern сжат с 3×3 до 3×2 (3 железные плиты → 2 кованых листа, логичнее для «корпуса» packager'а). `show_notification: false` (конвенция). **Ветка 1** скилла `recipe-override` (recipe-id в namespace `create`, без `BANNED_RECIPES`). Шейдинг-тегов не требуется: все id — прямые item-id (`tfc:metal/sheet/wrought_iron`, `create:cardboard_block`, `minecraft:redstone`). Recipe-id `create:crafting/logistics/packager` сохраняется, advancement `data/create/advancement/recipes/misc/crafting/logistics/packager.json` ссылается на тот же id — засчитывается без правок |
| `data/tfc_aeronautics/recipe/crafting/logistics/packager_steel_tight.json` | (новый рецепт, не замена) | тот же pattern `["SCS","R R"]`: S=`tfc_aeronautics:metal/tight_sheet/steel` (2×) + C=`create:cardboard_block` (1×) + R=`minecraft:redstone` (2×) → 1 `create:packager`. Параллельный вариант со стальным tight_sheet — игрок выбирает металл в верстаке (wrought iron через override в `create`, сталь через этот рецепт в `tfc_aeronautics`). `show_notification: false` (конвенция; прецедент — `data/tfc_aeronautics/recipe/crafting/kinetics/item_vault_steel_tight.json`). Шейдинг-тегов не требуется: все id — прямые item-id. Recipe-id `tfc_aeronautics:crafting/logistics/packager_steel_tight` **не** из Create-овского namespace, поэтому advancement Create `data/create/advancement/recipes/misc/crafting/logistics/packager.json` (привязан к `create:crafting/logistics/packager`) по этому пути **не** засчитывается — компромисс Datapack: один item, два recipe-id, advancement привязан к одному |
...
| `data/create/recipe/crafting/logistics/item_hatch.json` | Create shapeless `create:andesite_alloy + minecraft:iron_trapdoor` → 1 (recipe-id `create:crafting/logistics/item_hatch`) | тот же `minecraft:crafting_shapeless`: array `[tfc_aeronautics:metal/tight_sheet/steel, tfc:metal/sheet/wrought_iron]` (любой из двух) + array из 9 TFC металлических люков `[tfc:metal/trapdoor/bismuth_bronze, tfc:metal/trapdoor/black_bronze, tfc:metal/trapdoor/black_steel, tfc:metal/trapdoor/blue_steel, tfc:metal/trapdoor/bronze, tfc:metal/trapdoor/copper, tfc:metal/trapdoor/red_steel, tfc:metal/trapdoor/steel, tfc:metal/trapdoor/wrought_iron]` (любой) → 1 `create:item_hatch`. Мотивация: в TFC-сборке оригинал мёртв — `andesite_alloy` Create-only сплав (требует mechanical mixer), `minecraft:iron_trapdoor` заблокирован TFC через `neoforge:false` (`code_references/TerraFirmaCraft/src/generated/resources/data/minecraft/recipe/iron_trapdoor.json`); `tfc_aeronautics:metal/tight_sheet/steel` — аэронавтический стальной лист (tier 4 steel через TFC anvil или pressing); `tfc:metal/sheet/wrought_iron` — кованый лист через TFC anvil. inline-array — синтаксис vanilla 1.21.1 «любой из» (прецедент: `tree_fertilizer.json` в самом Create). `show_notification: false` (конвенция). **Ветка 1** (recipe-id в namespace `create`, без `BANNED_RECIPES`). Шейдинг-тегов не требуется: все 11 id — прямые item-id (`tfc_aeronautics:metal/tight_sheet/steel` зарегистрирован в `src/main/java/ru/tfc_aeronautics/metal/TightSheet.java`, `tfc:metal/sheet/wrought_iron` используется в `data/tfc_aeronautics/recipe/anvil/saw_blade.json`, 9 `tfc:metal/trapdoor/<metal>` имеют модели и `tfc/item_heat/<metal>/trapdoor.json` entries). Recipe-id `create:crafting/logistics/item_hatch` сохраняется, advancement Create (если есть) засчитывается без правок |
